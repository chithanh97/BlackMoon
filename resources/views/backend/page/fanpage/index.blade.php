...
@section('title', 'Danh sách Fanpage')
@section('content')
				...
				<div class="white_card_header">
					<div class="box_header m-0">
						...
						<div class="header_more_tool">
							<div class="dropdown">
								<span class="" id="">
									<button class="btn btn-primary add-button"><a href="{{ route('fanpage.add') }}" class='text-light'><i class="fa fa-plus-circle text-light" aria-hidden="true"></i> Thêm</a></button>
								</span>
							</div>
						</div>
					</div>
						...
						<table class="table pt-0 table-striped" id='tableData'>
							...
							<tbody>
								...
								<tr>
									...
									<td>
										<div class="{{ $value->status == 1 ? 'active' : ''}} status changestatus" data-table='fanpage' data-field='status' data-id='{{ $value->id }}'></div>
									</td>
									<td class="column-action">
										<a href="{{ route('fanpage.edit', $value->id) }}" class="btn-edit">
											<i class="fa fa-pencil"></i>
										</a>
										<a href="javascript:if(confirm('Bạn chắc chắn muốn xóa?')) window.location.href = '{{ route('fanpage.delete', $value->id) }}' ;
											" class="btn-delete">
...
<script>
	...
	function deleteAll(){
		if(confirm('Bạn chắc chắn muốn xóa?')) {
			...
			window.location.href = '{{ route("fanpage.deleteall") }}?value=' + JSON.stringify(value);
		}
	}
</script>
@endpush