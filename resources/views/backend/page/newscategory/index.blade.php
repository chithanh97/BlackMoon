				...
				<div class="white_card_header">
					<div class="box_header m-0">
						...
						<div class="header_more_tool">
							<div class="dropdown">
								<span class="" id="">
									<button class="btn btn-primary add-button"><a href="{{ route('newscategory.add') }}" class='text-light'><i class="fa fa-plus-circle text-light" aria-hidden="true"></i> Thêm</a></button>
								</span>
							</div>
						</div>
					</div>
								...
								<tr>
									...
									<td>
										<button class="btn btn-info">
											{{ getParentCate($parent, $value->parent) }}
										</button>
									</td>
									<td>{{ $value->sort == '' ? 0 : $value->sort}}</td>
									<td>
										<div class="{{ $value->hot == 1 ? 'active' : ''}} field-status changestatus" data-table='newscategory' data-field='hot' data-id='{{ $value->id }}'></div>
									</td>
									<td>
										<div class="{{ $value->status == 1 ? 'active' : ''}} status changestatus" data-table='newscategory' data-field='status' data-id='{{ $value->id }}'></div>
									</td>
									<td class="column-action">
										<a href="{{ route('newscategory.edit', $value->id) }}" class="btn-edit">
											<i class="fa fa-pencil"></i>
										</a>
										<a href="
										javascript:if(confirm('Bạn chắc chắn muốn xóa?')) window.location.href = '{{ route('newscategory.delete', $value->id) }}' ;
										" class="btn-delete">
...
<script>
	...
	function deleteAll(){
		if(confirm('Bạn chắc chắn muốn xóa?')) {
			...
			window.location.href = '{{ route("newscategory.deleteall") }}?value=' + JSON.stringify(value);
		}
	}
</script>
@endpush